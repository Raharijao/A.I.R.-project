...
#icons for different air quality groups
icon_map = {
'Good': r"icons/s1.png",
...
for i in range(0,len(max_pollutant_info)):
  ...
  for j in range(max_pollutant_info[i][1],len(locations[i][1])):
    popup_text =  popup_text + '<b>' + locations[i][1]['ParameterName'].values.tolist()[j] + '</b>'  + ': ' + str(locations[i][1]['AQI'].values.tolist()[j]) + ' (' + locations[i][1]['CategoryName'].values.tolist()[j]+ ')'
...
m.save(f'hourly_maps/map_{ct.year}_{ct.month}_{ct.day}_{ct.hour}.html')
